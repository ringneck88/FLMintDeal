...
class StrapiApp {
    ...
    async loadAdminTrads() {
        const adminLocales = await Promise.all(this.configurations.locales.map(async (locale)=>{
            try {
                const { default: data } = await __variableDynamicImportRuntime2__(`./translations/${locale}.js`);
                return {
                    data,
                    locale
                };
                ...
                try {
                    const { default: data } = await __variableDynamicImportRuntime3__(`./translations/${locale}.json`);
                    return {
                        data,
                        locale
                    };
