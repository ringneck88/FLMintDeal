...
var contentType = (({ strapi })=>{
    ...
    const buildContentTypeFilters = (contentType)=>{
        const utils$1 = strapi.plugin('graphql').service('utils');
        const extension = strapi.plugin('graphql').service('extension');
        const { getFiltersInputTypeName, getScalarFilterInputTypeName } = utils$1.naming;
        const { isStrapiScalar, isRelation, isComponent } = utils$1.attributes;
        ...
        return nexus.inputObjectType({
            ...
            definition (t) {
                ...
                .filter(([attributeName])=>!utils.contentTypes.isPrivateAttribute(contentType, attributeName))// Remove attributes that have been disabled using the shadow CRUD extension API
